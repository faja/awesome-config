...
-- terminal = "xterm & sleep .8s && transset -a"
terminal = "xterm"
...
vicious.register(batwidget, vicious.widgets.bat, "☢ $2%", 65, "BAT0")
...
--netup   = widget({ type = "textbox" })
--netdown = widget({ type = "textbox" })
...
--    end
    -- Down
--    netdown.text = string.format("%10s", format(down))
    -- Up
--    return string.format("%08s", format(up))
--  end, 5)
...
vicious.register(volwidget, vicious.widgets.volume, " $1%", 15, "Master")
